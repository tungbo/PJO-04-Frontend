  ...
  return (
    <div className="flex flex-wrap bg-orange-100">
      <div className="w-1/2 flex justify-center items-center">
        <img
          width={500}
          src={`${process.env.REACT_APP_URL_BACK_IMG}/${productDetail.imgPiza}`}
          alt=""
        />
      </div>
      <div className="w-1/2 my-auto">
        <h1 className="text-5xl font-semibold">{productDetail.namePiza}</h1>
        <p className="text-3xl pt-4 font-semibold text-red-500">
          ${productDetail.Price}
        </p>
        <p className="text-xl opacity-75">{productDetail.Description}</p>
        <div className="flex items-center">
          <b className="text-xl">Categories</b>:{" "}
          <p className="pl-4">Pizzas,Popular Dishes</p>
        </div>
        <div>
          <b>Size</b>: <p>{productDetail.idSize}</p>
        </div>
        <div className="flex">
          <div className="flex px-4 ">
            <input
              value={quantity}
              onInput={(e) => hanldeOnchangeInput(e)}
              className="w-10 pl-3 rounded-2xl py-4 text-xl focus:outline-none"
              type="text"
            />
          ...
          <button
            ...
            className="bg-button ml-4 text-2xl px-7 py-3 rounded-3xl"
          >
            Add To Cart
          </button>
        </div>
